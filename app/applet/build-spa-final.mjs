...
const keyExportDetailsRegex = /<section [^>]*id="whyus"[^>]*>([\s\S]*?)<\/section>/;
const keyExportDetailsMatch = homeSections.match(keyExportDetailsRegex);
let keyExportDetails = keyExportDetailsMatch ? `<section class="section" id="whyus" style="padding-bottom: 2rem;">${keyExportDetailsMatch[1]}</section>` : '';
    ...
    <div class="container hero-layout">
        ...
        <div class="hero-scrolling-content">
            ...
            <div style="background: white; border: 1px solid var(--border); border-radius: 8px; padding: 25px; box-shadow: 0 4px 6px rgba(0,0,0,0.02);">
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                    <span style="font-size: 1.3rem;">🚢</span>
                    <h3 style="font-size: 1.1rem; color: var(--text-main); margin: 0; font-family: var(--f-sans);">Seamless Global Logistics</h3>
                </div>
                <p style="font-size: 0.95rem; color: var(--text-muted); line-height: 1.5;">Benefit from our streamlined export processes. We ensure secure, hassle-free international shipping direct to your designated destination worldwide.</p>
            </div>
        </div>
    </div>
</section>
...
let aboutContent = extractMain(aboutHtml).replace('<section class="section">', '<section id="about" class="section" style="padding-top: 80px;">');
let productsContent = extractMain(productsHtml).replace('<section class="section">', '<section id="products" class="section" style="padding-top: 80px; padding-bottom: 80px; background: var(--surface);">');
let contactContent = extractMain(contactHtml).replace('<section class="section">', '<section id="contact" class="section" style="padding-top: 80px; padding-bottom: 80px;">');
